...
private fun calculateLockoutDelayMillis(failedAttempts: Int): Long {
    if (failedAttempts < 3) return 0L
    val extraFailures = failedAttempts - 3

    // cap shift to avoid overflow and because we cap anyway at MAX_LOCKOUT_MILLIS
    val safeShift = min(extraFailures, 5)
    val doubledDelay = BASE_LOCKOUT_MILLIS * (1L shl safeShift)
    return min(doubledDelay, MAX_LOCKOUT_MILLIS)
}
...
fun AppLockDialog(onSucceed: () -> Unit, onDismiss: () -> Unit) =
    Dialog(onDismiss, DialogProperties(true, false)) {
        ...
        var remainingSeconds by rememberSaveable {
            mutableIntStateOf(calculateRemainingSeconds(lockoutUntil))
        }
        ...
        fun unlock() {
            ...
            if (input.hash() == SP.lockPasswordHash) {
                ...
                val delayMillis = calculateLockoutDelayMillis(failedAttempts)
                if (delayMillis > 0L) {
                                ...
                                Icon(
                                    painter = painterResource(
                                        id = if (showPassword) {
                                            ...
                                            R.drawable.visibility_off_fill0
                                        }
                                    ),
