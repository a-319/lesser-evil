...
import lesser.evil.dpm.NetworkLog
import lesser.evil.dpm.SecurityEvent
import lesser.evil.dpm.SecurityEventWithData
import lesser.evil.dpm.transformSecurityEventData
...
class MyRepository(val dbHelper: MyDbHelper) {
    ...
    fun getPolicyToggles(): List<PolicyToggle> {
        val list = mutableListOf<PolicyToggle>()
    ...
    fun getPolicyToggle(id: Int): PolicyToggle? {
        dbHelper.readableDatabase.rawQuery(
            "SELECT * FROM policy_toggles WHERE id = ?", arrayOf(id.toString())
        ).use {
            if (it.moveToNext()) {
                return PolicyToggle(
                    it.getInt(0), it.getString(1), it.getInt(2) != 0, it.getInt(4) != 0,
