...
import lesser.evil.UserRestrictionCategory
...
fun UserRestrictionOptionsScreen(
    ...
    var infoDialog by remember { mutableStateOf<Restriction?>(null) }
    MyLazyScaffold(title, onNavigateUp) {
        items(items) { restriction ->
            Row(
                ...
                IconButton({ infoDialog = restriction }) {
    ...
    infoDialog?.let { restriction ->
        AlertDialog(
            title = { Text(stringResource(restriction.name)) },
            text = { Text(stringResource(restriction.info)) },
            confirmButton = {
                TextButton({ infoDialog = null }) { Text(stringResource(R.string.confirm)) }
            },
